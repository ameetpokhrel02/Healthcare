...
public class AdminLoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(username==null || username.trim().isEmpty() || password==null || password.trim().isEmpty() 
			...
			request.setAttribute("Error", "Username, password and role are required.");
			request.getRequestDispatcher("/view/AdminLogin.jsp").forward(request, response);
			return;
		}
		
		Healthcare healthcare = new Healthcare(username, password, role);
		healthcare.setEmail(username);
